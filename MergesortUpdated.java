...
//
	...
	public static void naturalMergesort(LinkedList l) {
		/*LinkedList some = stringToList("adbdaa");
		ListNode head = some.head();
		ListNode first = head;
		ListNode last = findMaximalIncreasingSubsequence(head);
		ListNode second = last.next();
		ListNode lasttwo = findMaximalIncreasingSubsequence(second);
		ListNode rest = lasttwo.last();
		last.setNext(null);
		lasttwo.setNext(null);
		merge(first,second);
		log("first" + first);
		first.last().setNext(rest);
		LinkedList th = stringToList("a");
		second = th.head();
		merge(first, second);
		log("second" + first);*/
		...
		//log(headOfSeq2.toString());
		//log(lastInSeq2.toString());
		/*log("h1" +headOfSeq1.toString());
		log("l1" +lastInSeq1.toString());
		log("h2"+headOfSeq2.toString());
		log("l2"+lastInSeq2.toString());
		log("r"+restOfList.toString());
		ListNode seq = lastInSeq2;
		seq.setNext(null);
		...
		lastInSeq2.last().prev().setNext(null);
		if (less(headOfSeq1.value(),headOfSeq2.value())) 
			merge(headOfSeq1,headOfSeq2);
		else 
			merge(headOfSeq2, headOfSeq1);
		log(restOfList.toString());*/
		while (lastInSeq2.next()!= null) {
			...
			log("bfr merge " + restOfList);
			log("bfr merge " + headOfSeq1);
			log("bfr merge " + headOfSeq2);
			...
			log("rest of " + restOfList);
			log("after 1 " +headOfSeq1);
			log("after 1 " +lastInSeq1);
			log("after 2 " +headOfSeq2);
			log("after 2 " +lastInSeq2);
			*/
			
			/*log("after merge 1" + headOfSeq1);
			log("after merge 1" + lastInSeq1);
			...
			if(restOfList.length() == 1) {
				...
				//log("after merge 2" + headOfSeq2);
				lastInSeq2 = findMaximalIncreasingSubsequence(headOfSeq2);
				//log("after merge 2" + lastInSeq2.last());
				//log("rest " +restOfList);
				lastInSeq2.setNext(null);
				merge(headOfSeq1, headOfSeq2);
				lastInSeq2 = null;
				log("rest " +restOfList);
				lastInSeq2.setNext(restOfList);
			*/
			}
			/*log("rstOf " +restOfList);
			//headOfSeq2 = restOfList;
			//lastInSeq2 = restOfList;
			log("head of seq2 " +headOfSeq2);

			log("last in seq2 " +lastInSeq2);
